...
function ($http, $location) {
    ...
    function modif(callback, user) {
        if (isLoginMocked === true) {
                userMock = user;
                callback(userMock);// On renvoie l'user mocké
        } else {
             //post = envoi au serveur la demande de modif
            $http.post("rest/modif", user)
            .success(function(data){
                userReal = data; //En cas de succes l'utilisateur reel est mis a jour puis on fais la callback de loginController (routage vers la vue)
                callback(data); //Renvoie l'utilisateur
            })
        ...
        function create(callback, user) {
        if (isLoginMocked === true) {
                userMock = user;
                callback(userMock);// On renvoie l'user mocké
        } else {
             //post = envoi au serveur la demande de modif
            $http.post("rest/add", user)
            .success(function(data){
                userReal = data; //En cas de succes l'utilisateur reel est mis a jour puis on fais la callback de loginController (routage vers la vue)
                callback(data); //Renvoie l'utilisateur
            })
    ...
    return {
        ...
        modif: function(callback, user){
            modif(callback, user);
        },

        create: function(callback, user){
